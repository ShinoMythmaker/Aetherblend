...
from rigify.rigs.limbs.limb_rigs import BaseLimbRig, SCRIPT_UTILITIES_OP_SNAP_IK_FK
...
class Rig(BaseLimbRig):
    """Human leg rig, AetherBlend style"""
    ...
    def initialize(self):
        ...
        if self.use_ik_toe:
            self.fk_name_suffix_cutoff = 4
            self.fk_ik_layer_cutoff = 5
        else:
    ...
    def prepare_bones(self):
        ...
        if self.params.rotation_axis == 'automatic':
            ...
            align_bone_x_axis(self.obj, orgs[4], -foot_x)
            ...
            if self.main_axis == 'x':
                ...
                align_bone_z_axis(self.obj, orgs[4], -foot_x)
    ...
    def rig_org_chain(self):
        orgs = self.bones.org.main
        ik_targets = self.get_ik_output_chain() + self.get_tail_ik_controls()

        if len(orgs) > 4:
            ik_targets = [
                ik_targets[0],
                self.bones.mch.ik_knee,
                ik_targets[1],
                ik_targets[2],
                *self.get_tail_ik_controls(),
            ]

        for i, (org, fk) in enumerate(zip(orgs, self.bones.ctrl.fk)):
            ik = ik_targets[i] if i < len(ik_targets) else None
            self.rig_org_bone(i, org, fk, ik)
    ...
    class MchBones(BaseLimbRig.MchBones):
        ...
        ik_knee: str                   # Intermediate MCH bone for the knee in the IK chain.
    ...
    def get_ik_fk_position_chains(self):
            ...
            if len(self.bones.org.main) > 4:
                ...
                fk_chain = self.bones.ctrl.fk[0:len(ik_chain) + len(tail_chain)]
    ...
    def make_ik_control_bone(self, orgs):
        name = self.copy_bone(orgs[3], make_derived_name(orgs[3], 'ctrl', '_ik'))
    ...
    def make_ik_ctrl_widget(self, ctrl):
        ...
        if self.pivot_type != 'TOE':
            ...
            org = self.get_bone(self.bones.org.main[4])
    ...
    def make_ik_spin_bone(self, orgs: list[str]):
        name = self.copy_bone(orgs[3], make_derived_name(orgs[3], 'ctrl', '_spin_ik'))
        put_bone(self.obj, name, self.get_bone(orgs[4]).head, matrix=self.ik_matrix, scale=0.5)
        return name
    ...
    def make_roll_mch_chain(self):
        orgs = self.bones.org.main
        self.bones.mch.heel = self.make_roll_mch_bones(orgs[3], orgs[4], self.bones.org.heel)
    ...
    def rig_ik_mch_end_bone(self, mch_ik: str, mch_target: str, ctrl_pole: str, chain=3):
    ...
    def configure_fk_control_bone(self, i, ctrl, org):
        super().configure_fk_control_bone(i, ctrl, org)

        # For 5-bone leg, lock location on foot (i == 3) instead of base class default (i == 2)
        if i == 3:
    ...
    def make_ik_mch_chain(self):
        ...
        self.bones.mch.ik_knee = self.make_ik_mch_knee_bone(orgs)
        self.bones.mch.ik_target = self.make_ik_mch_target_bone(orgs)
        self.bones.mch.ik_end = self.copy_bone(orgs[2], make_derived_name(orgs[2], 'mch', '_ik'))

    def make_ik_mch_knee_bone(self, orgs):
        return self.copy_bone(orgs[1], make_derived_name(orgs[1], 'mch', '_ik_knee'))

    def make_ik_mch_target_bone(self, orgs):
        return self.copy_bone(orgs[3], make_derived_name(orgs[3], 'mch', '_ik_target'))
    ...
    def parent_ik_mch_chain(self):
        ...
        self.set_bone_parent(self.bones.mch.ik_knee, self.bones.ctrl.ik_base)
        self.set_bone_parent(self.bones.mch.ik_end, self.bones.mch.ik_knee)
    ...
    def rig_ik_mch_chain(self):
        ...
        self.rig_ik_mch_stretch_limit(
            mch.ik_target, mch.follow, input_bone,
            self.ik_input_head_tail, 3
        )
        self.rig_ik_mch_end_bone(
            mch.ik_end, mch.ik_target, self.bones.ctrl.ik_pole, chain=3
        )
...
def create_sample(obj):
    ...
    bone.tail[:] = 0.0980, -0.0286, 0.7546
    bone.roll = 0.0000
    bone.use_connect = False
    bones['thigh.L'] = bone.name
    bone = arm.edit_bones.new('knee.L')
    bone.head[:] = 0.0980, -0.0286, 0.7546
    ...
    pbone.rigify_type = 'limbs.ablend_leg'
